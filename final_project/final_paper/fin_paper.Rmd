...
An F-test on the joint significance of country and time finds that we can reject at the 1% significance level (p = `r round(Fstat,4)`) the null hypothesis that time-period and country have no effect on sentiment. Interpreting the variation between countries and time periods at this aggregated level, as it is hard to make assumptions about how positive or negative sentiment expressed in tweets relates to opinions about political events or persons.
...
We focus in this paper on some specific search queries, though we also release our dataset aggregated at the country level in [interactive form](http://mcallaghan.github.io/col_res_proj/), where users can investigate any queries during any of the periods.
...
sentiment_time(merged_corpus_daily,"2015","memorandum_greece",clist=c("GR","DE"))
