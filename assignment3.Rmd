...
```{r include=FALSE}
pkgs <- c('twitteR', 'tm.lexicon.GeneralInquirer', 'tm.plugin.sentiment', 'dplyr', 'ggplot2', 'stringr', 'wordcloud', 'tm')
...
sum_q_loc_table <- senti_geo_corpus %>%
  ...
  summarise(
    n = length(query),
    percent = n/length(senti_geo_corpus$id)*100
  )
...
This table shows the distribution of all tweets over countries. Not many countries account for more than 1% of all tweets (Austria, Belgium, Canada, Germany, Estonia, GB, Greece, Italy, Netherlands, US and Venezuela). This group seems to be a mix of high twitter user numbers and affectedness/involvement/relation to the events in Greece. Most probably reflecting high user numbers in the US, the US seems to be the origin of most tweets regarding the European sovereign-debt crisis. However, when controlling for period, the US resembles an atypical case. While for most other countries, tweet number on the topic increase over the years, they decrease substantially in the US.

### Timeless against other queries

### Summary Charts

We argued earlier that it would be analytically helpful to distinguish between two groups of queries/tweets by high and low return rates. The following charts give an overview over the developments over time for both groups for all three time periods. Extreme points in these charts point to certain events. Also, the emergence of certain hashtags can be observed.
...
p1 <- timegraph(corpus_small,"2010-04-15-2010-05-15.txt")
ggtitle("Less frequently used words, 2010")
p2 <- timegraph(corpus_small,"2012-02-07-2012-03-31.txt")
ggtitle("Less frequently used words, 2012")
p3 <- timegraph(corpus_small,"2015-07-01-2015-08-25.txt")
ggtitle("Less frequently used words, 2015")
multiplot(p1, p2, p3, cols = 3)
p4 <- timegraph(corpus_large,"2010-04-15-2010-05-15.txt")
ggtitle("More frequently used words, 2010")
p5 <- timegraph(corpus_large,"2012-02-07-2012-03-31.txt")
ggtitle("More frequently used words, 2012")
p6 <- timegraph(corpus_large,"2015-07-01-2015-08-25.txt")
...
```
The word cloud gives us an overall picture of the words used by twitter users tweeting about the Greek memorandums. To a certain degree in this case a word cloud is redundant, because it mainly returns our query turns. However, it also indicates the prevalence of specific terms, and further, which other terms are mentioned regularly in those tweets. 
...
However, here is what the sentiment 
...
Our current sentiment analysis does not return reasonable results. The 
