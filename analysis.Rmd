...
by_country_by_year <- merged_corpus_europe %>%
  ...
  summarise(
    ...
    greece+debt = mean(stotal[query=="greece+debt"],na.rm=TRUE),
    greek+crisis = mean(stotal[query=="greek+crisis"],na.rm=TRUE),
    eurogroup = mean(stotal[query=="eurogroup"],na.rm=TRUE),
    bailout+greece = mean(stotal[query=="bailout+greece"],na.rm=TRUE),
    imf+greece = mean(stotal[query=="imf+greece"],na.rm=TRUE),
    syriza = mean(stotal[query=="syriza"],na.rm=TRUE),
    austerity+greece = mean(stotal[query=="austerity+greece"],na.rm=TRUE),
    germany_greece = mean(stotal[query=="germany+greece"],na.rm=TRUE),
    germany_greece = mean(stotal[query=="germany+greece"],na.rm=TRUE),
    germany_greece = mean(stotal[query=="germany+greece"],na.rm=TRUE)
  )
...
ggplot(a) +
  aes(long,lat,group=group,fill=eucrisis) +
  scale_fill_gradientn(colours=c("red","white","green"),limits=c(-2,2)) +
  geom_polygon() +
  #scale_fill_continuous(low="white",high="green",na.value="grey") +
  geom_path(color="grey",size=0.1) +
